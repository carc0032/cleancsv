...
# ============================
# Logging (Render-friendly JSON lines)
# ============================
    ...
    <div class="hero">
      <div>
        ...
        <p class="sub">
          ...
          Built for messy real-world exports: extra columns, missing fields, weird delimiters.
        </p>
        ...
        <ul class="bullets">
          ...
          <li><b>Optional near-duplicate preview</b> so you can see what would be removed before doing it.</li>
        </ul>
      </div>
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      <p style="margin:0;">
        <b>Rows:</b> {{ rows }} &nbsp; <b>Columns:</b> {{ cols }}
      </p>
      ...
      <p class="muted" style="margin-top:12px;">
        $5 one-time download • Files are automatically deleted after {{ retention }} minutes
      </p>
    </div>
...
<body>
  <div class="wrap">
    ...
    <footer>
      Files auto-delete after {{ retention }} minutes • Support: {{ support_email }}
    </footer>
  </div>
</body>
</html>
...
def read_csv_lenient(path: Path, delimiter: str) -> tuple[pd.DataFrame, list[str], bool, list[int]]:
    """
    Pads/truncates rows to match header. Enforces MAX_ROWS/MAX_COLS.
    """
    import_log: list[str] = []
    rows: list[list[str]] = []
...
def download_original(job_id: str):
    ...
    log_event("download_original_served", job_id=job_id)
...
def download(job_id: str):
    ...
    # If payment is pending, don't send user back to Stripe (prevents double-pay anxiety)
...
def success():
    ...
    if sess.payment_status == "paid" and (sess.metadata or {}).get("job_id") == job_id:
        ...
        return render_template_string(SUCCESS_HTML, job_id=job_id, retention=RETENTION_MINUTES, support_email=SUPPORT_EMAIL)
